...
public:
  ...
  {
    if ( status_codes_.find( statusCode ) == status_codes_.end() ) {
      return "unknown";
    }
    return status_codes_[statusCode];
  }

private:
  void Set( int32_t statusCode, std::string message ) { status_codes_[statusCode] = std::move( message ); }
